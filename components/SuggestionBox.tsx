...
const translations = {
  en: {
    language: 'English', otherLanguage: 'Kinyarwanda', title: 'Suggestion Box', intro: 'Your feedback helps us improve the care and service we provide.',
    feedbackType: 'Feedback type', selectOne: 'Select one', suggestion: 'Suggestion', complaint: 'Complaint', compliment: 'Compliment', other: 'Other',
    department: 'Department / Service', selectDepartment: 'Select department / service', pleaseSpecify: 'Please specify', typeDepartment: 'Type the department or service',
    message: 'Your message', messagePlaceholder: 'Tell us what you would like KHMC to know...', anonymous: 'Submit anonymously', name: 'Name', yourName: 'Your name',
    phone: 'Phone', phonePlaceholder: '+250 7XX XXX XXX', submitting: 'Submitting...', submit: 'Submit Feedback',
    privacy: 'Your feedback is treated respectfully. You may submit anonymously.', thankYou: 'Thank you', received: 'Your feedback has been received by KHMC.', another: 'Submit another',
    submitError: 'Unable to submit. Server returned', connectionError: 'Unable to connect to the server. Please try again.',
  },
  rw: {
    language: 'Kinyarwanda', otherLanguage: 'English', title: 'Agasanduku k\'Ibitekerezo', intro: 'Ibitekerezo byanyu bidufasha kunoza serivisi n\'ubuvuzi dutanga.',
    feedbackType: 'Ubwoko bw\'igitekerezo', selectOne: 'Hitamo kimwe', suggestion: 'Igitekerezo', complaint: 'Ikirego', compliment: 'Ishimwe', other: 'Ibindi',
    department: 'Ishami / Serivisi', selectDepartment: 'Hitamo ishami / serivisi', pleaseSpecify: 'Sobanura neza', typeDepartment: 'Andika ishami cyangwa serivisi',
    message: 'Ubutumwa bwawe', messagePlaceholder: 'Tubwire icyo wifuza ko KHMC imenya...', anonymous: 'Ohereza utagaragaje amazina', name: 'Amazina', yourName: 'Andika amazina yawe',
    phone: 'Telefoni', phonePlaceholder: '+250 7XX XXX XXX', submitting: 'Birimo koherezwa...', submit: 'Ohereza Igitekerezo',
    privacy: 'Igitekerezo cyawe kizafatwa mu cyubahiro. Ushobora kugitanga utagaragaje amazina.', thankYou: 'Murakoze', received: 'Igitekerezo cyawe cyakiriwe na KHMC.', another: 'Ohereza ikindi',
    submitError: 'Ntibishoboye koherezwa. Seriveri yagaruye', connectionError: 'Ntibishoboye guhuza na seriveri. Ongera ugerageze.',
  },
} as const;
  ...
  if (sent) return (
    <main className="page">
      ...
      <section className="card success">
        ...
        <h1>{t.thankYou}</h1><p>{t.received}</p>
      ...
      <form className="card form" onSubmit={submit}>
        <label>{t.feedbackType}
          <select name="type" required>
            <option value="">{t.selectOne}</option><option value="Suggestion">{t.suggestion}</option><option value="Complaint">{t.complaint}</option><option value="Compliment">{t.compliment}</option><option value="Other">{t.other}</option>
          </select>
        </label>
        <label>{t.department}
          <select name="department" required defaultValue="" onChange={(e) => setOtherDepartment(e.target.value === 'Other')}>
            <option value="">{t.selectDepartment}</option>
            <option value="Reception">Reception</option><option value="Consultation">Consultation</option><option value="Laboratory">Laboratory</option><option value="Pharmacy">Pharmacy</option><option value="Nursing">Nursing</option><option value="Dental">Dental</option><option value="Maternity">Maternity</option><option value="Emergency">Emergency</option><option value="Outpatient">Outpatient</option><option value="Administration">Administration</option><option value="Other">{t.other}</option>
          </select>
        </label>
