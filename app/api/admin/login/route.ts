...
export async function POST(request: Request) {
  try {
    ...
    const wanted = String(username || 'admin').trim().toLowerCase() || 'admin';
    if (!password) return NextResponse.json({ error: 'Password is required' }, { status: 400 });

    const supabase = adminDb();
    let { data: admins, error } = await supabase.from('admin_users').select('id,username,name,password_hash,is_active').eq('is_active', true);
    ...
    if ((!admins || admins.length === 0) && process.env.ADMIN_PASSWORD && wanted === 'admin' && password === process.env.ADMIN_PASSWORD) {
      const created = await supabase.from('admin_users').insert({ username:'admin', name:'Administrator', password_hash:hashPassword(password), is_active:true }).select('id,username,name,password_hash').single();
