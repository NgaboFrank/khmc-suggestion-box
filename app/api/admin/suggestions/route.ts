...
function db() {
  ...
  return createClient(url, key, { db: { schema: 'public' }, auth: { autoRefreshToken: false, persistSession: false, detectSessionInUrl: false } });
}
...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ suggestions: data || [] }, { headers: { 'Cache-Control': 'no-store, no-cache, must-revalidate' } });
  } catch (error: any) {
...
export async function DELETE(request: Request) {
  ...
  if (!admin.is_owner) return NextResponse.json({ error: 'Only the owner can delete suggestions' }, { status: 403 });
  try {
